 ...
 * @Date: 2022-09-19 11:45:58
 * @LastEditors: Marvelousp4 573335988@qq.com
 * @LastEditTime: 2022-10-19 15:45:03
 * @FilePath: /CSAPP/07-CacheLab/cachelab-handout/csim.c
 * @Description:
 *
...
/*
   cache has 2^s group, every group has E lines
       1. E = 1
       2. 1 < E < C/B
       3. E = C/B
    1. valid bit
        1: valid information
        0: nothing valuable
    2. tag: when cpu reads some address's content compare tag bit to determine wheter hit
        t = m - (s + b)
    3. data block B
 */
#include "cachelab.h"
...
void update(unsigned address)
{
    ...
    unsigned targetSet = ((maskSet) & (address >> B)); // 获取 s，考虑先右移 b 位，再用无符号 0xFF... 右移后进行与操作将 tag 抹去。
    unsigned targetTag = address >> (S + B); // tag 很容易得到，右移 (b + s) 位即可
